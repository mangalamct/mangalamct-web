...
const Success = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
        window.location.href = `mangalamct://(modals)/agent/PaymentStatusScreen/?status=success&id=${searchParams.get('id')}`;
      }, 5000);
    // fetchPaymentData();
  }, [searchParams]);
...
const handleGoBack = () => {
  ...
  window.location.href = `mangalamct://(modals)/agent/PaymentStatusScreen/?status=success&id=${transactionId}`;
};
